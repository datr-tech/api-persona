...
export const organisationRoleControllerReadOrganisationRole: IOrganisationRoleControllerReadOrganisationRole =
  async ({ organisationRoleId }) => {
    ...
    try {
       ...
       */
      stat.error = false;
      stat.payload = { organisationRoleModel };

      /*
       * Cast the response object to 'IOrganisationRoleControllerReadOrganisationRoleOutputSuccess',
       * where the casting interface is a component of the binary union type
       * 'IOrganisationRoleControllerReadOrganisationRoleOutput'.
       */
