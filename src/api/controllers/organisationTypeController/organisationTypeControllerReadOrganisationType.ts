...
export const organisationTypeControllerReadOrganisationType: IOrganisationTypeControllerReadOrganisationType =
  async ({ organisationTypeId }) => {
    ...
    try {
       ...
       */
      stat.error = false;
      stat.payload = { organisationTypeModel };

      /*
       * Cast the response object to 'IOrganisationTypeControllerReadOrganisationTypeOutputSuccess',
       * where the casting interface is a component of the binary union type
       * 'IOrganisationTypeControllerReadOrganisationTypeOutput'.
       */
