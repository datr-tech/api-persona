...
import {
  IOrganisationRoleControllerReadOrganisationRoleOutputError as IControllerError,
  IOrganisationRoleControllerReadOrganisationRoleOutputSuccess as IControllerSuccess,
...
/**
 * @name					organisationRoleRouterReadOrganisationRole
 *
 * @description		The 'readOrganisationRole' router for 'organisationRole', whose expected
 *                inputs have been defined within the following schema:
 *                'organisationRoleValidationSchemaReadOrganisationRole'.
 *
...
export const organisationRoleRouterReadOrganisationRole = Router(options).get(
  ...
  async (req: Request, res: Response) => {
    ...
    try {
       ...
       * defined within 'organisationRoleValidationSchemaReadOrganisationRole'.
      ...
      const stat = await organisationRoleController.readOrganisationRole(validatedParams);
       ...
       * 'organisationRoleModel' from 'stat.payload' and return
       * it with an appropriate status code.
       */

      const { organisationRoleModel, responseStatusCode } = (stat as IControllerSuccess)
        .payload;
      res.status(responseStatusCode).send({ organisationRoleModel });
    } catch (error) {
      /*
