...
import {
  IUserSessionControllerReadUserSessionOutputError as IControllerError,
  IUserSessionControllerReadUserSessionOutputSuccess as IControllerSuccess,
...
/**
 * @name					userSessionRouterReadUserSession
 *
 * @description		The 'readUserSession' router for 'userSession', whose expected
 *                inputs have been defined within the following schema:
 *                'userSessionValidationSchemaReadUserSession'.
 *
...
export const userSessionRouterReadUserSession = Router(options).get(
  ...
  async (req: Request, res: Response) => {
    ...
    try {
       ...
       * defined within 'userSessionValidationSchemaReadUserSession'.
      ...
      const stat = await userSessionController.readUserSession(validatedParams);
       ...
       * 'userSessionModel' from 'stat.payload' and return
       * it with an appropriate status code.
       */

      const { userSessionModel, responseStatusCode } = (stat as IControllerSuccess)
        .payload;
      res.status(responseStatusCode).send({ userSessionModel });
    } catch (error) {
      /*
