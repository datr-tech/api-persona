...
import {
  IUserSessionControllerDeleteUserSessionOutputError as IControllerError,
  IUserSessionControllerDeleteUserSessionOutputSuccess as IControllerSuccess,
...
/**
 * @name					userSessionRouterDeleteUserSession
 *
 * @description		The 'deleteUserSession' router for 'userSession', whose expected
 *                inputs have been defined within the following schema:
 *                'userSessionValidationSchemaDeleteUserSession'.
 *
 ...
 *                controller, 'userSessionController', which, when successful, will return
 ...
 *                | Server error                | 500               |
 */
export const userSessionRouterDeleteUserSession = Router(options).delete(
  '/',
  ...
  async (req: Request, res: Response) => {
    ...
    try {
       ...
       * defined within 'userSessionValidationSchemaDeleteUserSession'.
      ...
      const stat = await userSessionController.deleteUserSession(validatedParams);
       ...
       * 'userSessionId' from 'stat.payload' and return
      ...
      res
        ...
        .send({ userSessionId: controllerResponsePayload['userSessionId'] });
    } catch (error) {
      /*
