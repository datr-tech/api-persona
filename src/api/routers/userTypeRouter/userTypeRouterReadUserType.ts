...
import {
  IUserTypeControllerReadUserTypeOutputError as IControllerError,
  IUserTypeControllerReadUserTypeOutputSuccess as IControllerSuccess,
...
/**
 * @name					userTypeRouterReadUserType
 *
 * @description		The 'readUserType' router for 'userType', whose expected
 *                inputs have been defined within the following schema:
 *                'userTypeValidationSchemaReadUserType'.
 *
 ...
 *                controller, 'userTypeController', which, when successful, will return
...
export const userTypeRouterReadUserType = Router(options).get(
  ...
  async (req: Request, res: Response) => {
    ...
    try {
       ...
       * defined within 'userTypeValidationSchemaReadUserType'.
      ...
      const stat = await userTypeController.readUserType(validatedParams);
       ...
       * 'userTypeModel' from 'stat.payload' and return
       * it with an appropriate status code.
       */

      const { userTypeModel, responseStatusCode } = (stat as IControllerSuccess).payload;
      res.status(responseStatusCode).send({ userTypeModel });
    } catch (error) {
      /*
