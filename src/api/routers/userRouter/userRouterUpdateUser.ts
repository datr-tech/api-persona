...
import {
  IUserControllerUpdateUserOutputError as IControllerError,
  IUserControllerUpdateUserOutputSuccess as IControllerSuccess,
} from '@app-ap/interfaces/api/controllers';
import { userValidationSchemaUpdateUser } from '@datr.tech/cargo-router-validation-schemas-persona';
...
/**
 * @name					userRouterUpdateUser
 *
 * @description		The 'updateUser' router for 'user', whose expected
 *                inputs have been defined within the following schema:
 *                'userValidationSchemaUpdateUser'.
 *
...
export const userRouterUpdateUser = Router(options).patch(
  ...
  async (req: Request, res: Response) => {
    ...
    try {
       ...
       * defined within 'userValidationSchemaUpdateUser'.
      ...
      const stat = await userController.updateUser({ userId, payload });
       ...
       * 'userId' from 'stat.payload' and return
      ...
      res
        ...
        .send({ userId: controllerResponsePayload['userId'] });
    } catch (error) {
      /*
