...
import {
  IUserControllerDeleteUserOutputError as IControllerError,
  IUserControllerDeleteUserOutputSuccess as IControllerSuccess,
...
/**
 * @name					userRouterDeleteUser
 *
 * @description		The 'deleteUser' router for 'user', whose expected
 *                inputs have been defined within the following schema:
 *                'userValidationSchemaDeleteUser'.
 *
 ...
 *                | Server error                | 500               |
 */
export const userRouterDeleteUser = Router(options).delete(
  '/',
  ...
  async (req: Request, res: Response) => {
    ...
    try {
       ...
       * defined within 'userValidationSchemaDeleteUser'.
      ...
      const stat = await userController.deleteUser(validatedParams);
